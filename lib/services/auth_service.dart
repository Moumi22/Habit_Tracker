...
class AuthService {
  ...
  Future<UserCredential> _loginWithWorkaround(
    String email,
    String password,
  ) async {
    try {
  ...
  Future<bool> isUserLoggedIn() async {
    try {
      ...
      print('Checking login state: userId=$userId, isLoggedIn=$isLoggedIn, lastLoginTime=$lastLoginTime');
      
      // Also check if Firebase user is still valid
      final firebaseUser = _auth.currentUser;
      print('Firebase current user: ${firebaseUser?.uid ?? 'null'}');
      
      // Check if session is not too old (optional - you can remove this if you want sessions to persist indefinitely)
      if (lastLoginTime != null) {
        final loginTime = DateTime.parse(lastLoginTime);
        final now = DateTime.now();
        final daysSinceLogin = now.difference(loginTime).inDays;
        
        // If it's been more than 30 days, consider session expired
      ...
      final result = userId != null && isLoggedIn && firebaseUser != null;
      print('Login state result: $result');
      return result;
    } catch (e) {
  ...
  Future<bool> restoreAuthState() async {
    try {
      print('Attempting to restore auth state...');
      ...
      // If no valid session, try to get current Firebase user
      final currentUser = _auth.currentUser;
      if (currentUser != null) {
        print('Found Firebase user, saving session');
        await saveUserSession(currentUser.uid);
        return true;
      }
      
      print('No valid auth state found');
      return false;
    } catch (e) {
