...
class HabitProvider extends ChangeNotifier {
    ...
    return _habits.where((habit) {
      if (habit.frequency == HabitFrequency.daily) {
        ...
        return habit.startDate == null ||
            (habit.startDate!.isBefore(
                  endOfWeek.add(const Duration(days: 1)),
                ) &&
                habit.startDate!.isAfter(
  ...
  Future<bool> markHabitCompleted(
    ...
    try {
      ...
      if (index != -1) {
        ...
        final updatedCompletionHistory = List<DateTime>.from(
          habit.completionHistory,
        )..add(date);
        final newStreak = _calculateStreak(
          updatedCompletionHistory,
          habit.frequency,
        );
  ...
  Future<bool> markHabitIncomplete(
    String userId,
    String habitId,
    DateTime date,
  ) async {
    try {
      ...
      if (index != -1) {
        final habit = _habits[index];
        final updatedCompletionHistory =
            habit.completionHistory
                .where(
                  (completionDate) =>
  ...
  Map<HabitCategory, Map<String, dynamic>> getCategoryStats() {
    ...
    for (final category in HabitCategory.values) {
      final categoryHabits =
          _habits.where((habit) => habit.category == category).toList();
      ...
      for (int i = 0; i < sortedDates.length; i++) {
        ...
        if (i == 0) {
          final weeksDifference =
              startOfCurrentWeek.difference(startOfCompletionWeek).inDays ~/ 7;
